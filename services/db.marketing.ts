...
export const addCouponDB = async (coupon: Coupon) => { 
    ...
    // Notificación automática de nuevo cupón
...
export const deleteCouponDB = async (id: string) => { 
    await deleteDoc(doc(firestore, COUPONS_COLLECTION, id)); 
};
...
export const addBannerDB = async (banner: Banner) => { 
    const { id, ...data } = banner; 
    await addDoc(collection(firestore, BANNERS_COLLECTION), cleanData(data)); 
};

export const deleteBannerDB = async (id: string) => { 
...
export const addBlogPostDB = async (post: BlogPost) => { 
    ...
    // Notificación de nueva entrada en el blog
    await sendNotificationToAll({
